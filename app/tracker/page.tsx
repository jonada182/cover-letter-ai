...
import { PiLinkThin, PiTrashThin, PiPencilThin, PiCalendarPlusThin, PiNoteThin } from "react-icons/pi"
...
export default function Page() {
  ...
  const [currentModalEvent, setCurrentModalEvent] = useState<JobApplicationEvent | null>(null)
  ...
  return (
    <div>
          ...
          <div key={jobApplication.id} className="flex flex-col bg-white border-t border-gray-200 first:border-0">
            <div className="flex items-center justify-stretch">
              <div className="flex-grow border-r border-gray-200 p-4">
                <div className="flex justify-between items-center">
                  <div className="text-sm font-bold text-blue-900 capitalize">{jobApplication.job_role}</div>
              ...
              <div className="w-1/3 md:w-1/4 flex justify-center items-center p-4 gap-2">
                ...
                {jobApplication.url && isValidURL(jobApplication.url) && <Link className="btn-icon" target="_blank" href={jobApplication.url}><PiLinkThin /></Link>}
                <button className="btn-icon" onClick={() => handleEditApplication(jobApplication)}><PiPencilThin /></button>
            ...
            <div>
              {jobApplication.events?.map((event, index) => {
                ...
                return (
                  <div className="flex gap-4 py-2 px-4 justify-between items-center border-t relative text-xs" key={index}>
                    <div className="p-2 bg-blue-200 rounded-sm text-xs uppercase w-24 text-center">{JobApplicationEventType[event.type]}</div>
                    ...
                    <button className="btn-icon" onClick={() => toggleEventNotes(event)}><PiNoteThin /></button>
                    <button className="btn-icon" onClick={() => deleteJobApplicationEvent(jobApplication.id, index)}><PiTrashThin /></button>
                  </div>
                )
              })}
            </div>
      ...
      <Modal isOpen={notesModalIsOpen} onClose={() => {setNotesModalIsOpen(false); setCurrentModalEvent(null);}} title={`Additional Notes - ${currentModalEvent?.description}`}>
        <div className="text-sm text-gray-600 my-4">{currentModalEvent?.additional_notes}</div>
      </Modal>
    </div>
  )
}

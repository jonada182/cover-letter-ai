...
describe("deleteJobApplication", () => {
  ...
  it("fetches data successfully from the API", async () => {
    ...
    const data = await deleteJobApplication({ jobApplicationId: testJobApplicationID, accessToken: testAccessToken, profileId: testProfileID })
  ...
  it("API returns with an error", async () => {
    ...
    expect(deleteJobApplication({ jobApplicationId: testJobApplicationID, accessToken: testAccessToken, profileId: testProfileID })).rejects.toThrow("oops!")
    expect(mockedAxios.delete).toHaveBeenCalled()
    expect(mockedAxios.delete).toHaveBeenCalledWith(`/job-applications/${testJobApplicationID}`, { headers: headers })
  })
})

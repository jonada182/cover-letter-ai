...
describe("getCareerProfile", () => {
  ...
  it("fetches data successfully from the API", async () => {
    ...
    const data = await getCareerProfile({ profileId: testProfileID, accessToken: testAccessToken })
    expect(mockedAxios.get).toHaveBeenCalled()
    expect(mockedAxios.get).toHaveBeenCalledWith("/career-profile", { headers: headers})
  ...
  it("API returns with an error", async () => {
    ...
    expect(getCareerProfile({ profileId: testProfileID, accessToken: testAccessToken })).rejects.toThrow("oops!")
    expect(mockedAxios.get).toHaveBeenCalled()
    expect(mockedAxios.get).toHaveBeenCalledWith("/career-profile", { headers: headers})
  })
})

...
const FormTextarea = (props: Props) => {
  return (
    <div className="flex flex-col max-w-lg justify-stretch mb-4">
      ...
      <textarea
        className={`flex-grow p-3 transition-all shadow-sm my-3 rounded border border-gray-200 focus:border-pink-600 outline-none ${props.large ? "h-64" : "h-36"} max-h-64`}
        name={props.name}
        id={props.name}
